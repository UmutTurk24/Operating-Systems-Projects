...
// SIGALARM and Setitimer
#define INTERVAL 100		/* number of milliseconds to go off */
...
void thread_init(int preemption_enabled) {
    ...
    // Setup sigalrm signal for calling thread_yield in given intervals
    if (signal(SIGALRM, (void (*)(int)) thread_yield_caller) == SIG_ERR) {
    ...
    // Setup sigusr signal for thread creation
    ...
    // Send a signal to myself, and specify a new stack. THIS IS NOT NEEDED
	stack_t new_stack;
	stack_t old_stack;
...
int thread_yield(void) {
    ...
    // Pause the timer for calling thread_yield
    setitimer(ITIMER_REAL, &zero_timer, &it_val);
